class SalesEntry {
    ...
    setupSalesBagCountKeyupHandler() {
        $(document).on("keyup", ".sales_bag_count", function () {
            ...
            const elements = document.querySelectorAll('.add_new_sales_custom_select');
            elements.forEach((element) => {
                const idAttr = element.getAttribute('id');
                const dynamicNumberFromId = idAttr.split('_')[0];
                if ($(`#${dynamicNumberFromId}_lot_number`).val() === $(`#${res[0]}_lot_number`).val()) {
                    const bagsValue = parseInt($(`#${dynamicNumberFromId}_bags`).val()) || 0;
                    bag_count += bagsValue;
                }
            });
    ...
    addSalesEntryRow() {
        ...
        let select_option = '<option selected="true" disabled="disabled">Choose Lot No</option>';
