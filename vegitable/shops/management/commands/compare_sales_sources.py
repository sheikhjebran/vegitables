...
def _normalize_sql_items(entry):
    rows = SalesBillItem.objects.filter(Sales_Bill_Entry=entry).order_by('id')
    return tuple(
        sorted(
            (
                str(row.arrival_goods.arrival_entry_id),
                str(row.arrival_goods_id),
                str(row.item_name),
                int(row.bags),
                round(float(row.net_weight), 4),
                round(float(row.rates), 4),
...
def _normalize_firebase_items(items):
    normalized = []
    for item in items:
        normalized.append(
            (
                str(item.get('arrival_entry_id', '')),
                str(item.get('arrival_goods_local_id', '')),
                str(item.get('item_name', '')),
                int(item.get('bags', 0)),
                round(float(item.get('net_weight', 0.0)), 4),
                round(float(item.get('rates', 0.0)), 4),
                round(float(item.get('amount', 0.0)), 4),
            )
        )
...
def build_sql_signature(row):
    return (
        int(row.shop_id),
        str(row.sales_bill_id),
        str(row.payment_type),
        str(row.customer_name),
        str(row.date),
        round(float(row.rmc), 4),
        round(float(row.commission), 4),
        round(float(row.cooli), 4),
        round(float(row.total_amount), 4),
        round(float(row.paid_amount), 4),
        round(float(row.balance_amount), 4),
        bool(row.Empty_data),
        _normalize_sql_items(row),
    )
...
def build_firebase_signature(row):
    return (
        ...
        round(float(row.balance_amount), 4),
        bool(row.empty_data),
        _normalize_firebase_items(list(row.items or [])),
    )


class Command(BaseCommand):
    help = 'Compare SQL SalesBillEntry rows against Firestore SalesBillEntry documents.'
    ...
    def handle(self, *args, **options):
        ...
        try:
            sql_rows = list(SalesBillEntry.objects.filter(Empty_data=False).order_by('id'))
        except OperationalError as exc:
            raise CommandError(self._build_source_db_error(exc)) from exc
        firebase_rows = list(SalesBillEntryDocument.objects.all())
        ...
        if sql_only or firebase_only:
            self.stdout.write(self.style.WARNING('Sales source mismatch detected.'))
            if mismatch_limit:
            ...
            raise CommandError('SQL and Firestore Sales records are not in parity.')

        self.stdout.write(self.style.SUCCESS('SQL and Firestore Sales records are in parity.'))
