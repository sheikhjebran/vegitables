...
class Command(BaseCommand):
    help = 'Backfill SQL FarmerLedger rows into Firestore for the feature-flagged farmer ledger slice.'
    ...
    def handle(self, *args, **options):
        ...
        if not repository.using_firebase():
            raise CommandError(
                'Set USE_FIREBASE_FARMER_LEDGER=True before running this command so writes target Firestore.'
            )

        queryset = FarmerLedger.objects.order_by('id')
        limit = options['limit']
        ...
        if options['dry_run']:
            self.stdout.write(f'Dry run: {len(rows)} FarmerLedger rows would be backfilled to Firestore.')
            return
        ...
        if failures:
            raise CommandError('One or more FarmerLedger rows failed during backfill.')
