...
class Command(BaseCommand):
    help = 'Compare SQL CustomerLedger rows against Firestore CustomerLedger documents.'
    ...
    def handle(self, *args, **options):
        ...
        try:
            sql_rows = list(CustomerLedger.objects.order_by('id'))
        except OperationalError as exc:
            raise CommandError(self._build_source_db_error(exc)) from exc
        firebase_rows = list(CustomerLedgerDocument.objects.all())
        ...
        if sql_only or firebase_only:
            self.stdout.write(self.style.WARNING('CustomerLedger source mismatch detected.'))
            if mismatch_limit:
            ...
            raise CommandError('SQL and Firestore CustomerLedger records are not in parity.')

        self.stdout.write(self.style.SUCCESS('SQL and Firestore CustomerLedger records are in parity.'))
