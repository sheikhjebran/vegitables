...
def _normalize_sql_histories(entry):
    rows = CreditBillHistory.objects.filter(credit_bill=entry).order_by('id')
    return tuple(
        sorted(
            (
                str(row.date),
...
def _normalize_firebase_histories(histories):
    normalized = []
    for history in histories:
        normalized.append(
            (
                str(history.get('date', '')),
                round(float(history.get('amount', 0.0)), 4),
                str(history.get('payment_mode', '')),
            )
        )
...
def build_sql_signature(row):
    return (
        ...
        str(row.sales_bill.sales_bill_id),
        round(float(row.initial_credit_bill_amount), 4),
        _normalize_sql_histories(row),
    )
...
def build_firebase_signature(row):
    return (
        ...
        round(float(row.initial_credit_bill_amount), 4),
        _normalize_firebase_histories(list(row.histories or [])),
    )


class Command(BaseCommand):
    help = 'Compare SQL CreditBillEntry rows against Firestore CreditBillEntry documents.'
    ...
    def handle(self, *args, **options):
        ...
        try:
            sql_rows = list(CreditBillEntry.objects.order_by('id'))
        except OperationalError as exc:
            raise CommandError(self._build_source_db_error(exc)) from exc
        firebase_rows = list(CreditBillEntryDocument.objects.all())
        ...
        if sql_only or firebase_only:
            self.stdout.write(self.style.WARNING('Credit source mismatch detected.'))
            if mismatch_limit:
            ...
            raise CommandError('SQL and Firestore Credit records are not in parity.')

        self.stdout.write(self.style.SUCCESS('SQL and Firestore Credit records are in parity.'))
