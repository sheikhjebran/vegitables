...
class Command(BaseCommand):
    help = 'Compare SQL FarmerLedger rows against Firestore FarmerLedger documents.'
    ...
    def handle(self, *args, **options):
        ...
        try:
            sql_rows = list(FarmerLedger.objects.order_by('id'))
        except OperationalError as exc:
            raise CommandError(self._build_source_db_error(exc)) from exc
        firebase_rows = list(FarmerLedgerDocument.objects.all())
        ...
        if sql_only or firebase_only:
            self.stdout.write(self.style.WARNING('FarmerLedger source mismatch detected.'))
            if mismatch_limit:
            ...
            raise CommandError('SQL and Firestore FarmerLedger records are not in parity.')

        self.stdout.write(self.style.SUCCESS('SQL and Firestore FarmerLedger records are in parity.'))
    ...
    def _build_source_db_error(self, exc):
        ...
        if 'no such table: shops_farmerledger' in message:
            return (
                'The local SQLite database does not contain the farmer ledger source table. Either run the SQL '
        ...
        return f'Source database error while reading FarmerLedger rows: {message}'