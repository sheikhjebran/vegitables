...
def _normalize_sql_items(entry):
    rows = PattiEntryList.objects.filter(patti=entry)
    return tuple(
        sorted(
            (
                str(row.item),
...
def build_sql_signature(row):
    return (
        ...
        str(row.farmer_name),
        round(float(row.total_weight), 4),
        round(float(row.hamali), 4),
        round(float(row.net_amount), 4),
...
def build_firebase_signature(row):
    return (
        ...
        round(float(row.hamali), 4),
        round(float(row.net_amount), 4),
        _normalize_items(list(row.items or [])),
    )


class Command(BaseCommand):
    help = 'Compare SQL PattiEntry rows against Firestore PattiEntry documents.'
    ...
    def handle(self, *args, **options):
        ...
        try:
            sql_rows = list(PattiEntry.objects.order_by('id'))
        except OperationalError as exc:
            raise CommandError(self._build_source_db_error(exc)) from exc
        firebase_rows = list(PattiEntryDocument.objects.all())
        ...
        if sql_only or firebase_only:
            self.stdout.write(self.style.WARNING('Patti source mismatch detected.'))
            if mismatch_limit:
            ...
            raise CommandError('SQL and Firestore Patti records are not in parity.')

        self.stdout.write(self.style.SUCCESS('SQL and Firestore Patti records are in parity.'))
